...
已实现：独立 Vue 项目、作品与章节管理、正文编辑、本机自动保存、故事概念与设定库、四类 AI 生成、预览后采纳、新版 JSON 作品导入导出。未实现：旧版数据迁移、桌面容器、自动生文工作流、榜单、拆书、统计、图片生成和完整的版本历史。后续应先把数据安全、旧版迁移和 Agnes 兼容性做扎实，再扩展旧版的外围模块。
