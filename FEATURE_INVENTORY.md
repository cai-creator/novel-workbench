...
已实现：独立 Vue 项目、作品书架与章节管理、正文编辑、本机自动保存、故事概念与设定库、四类 AI 生成、预览后采纳、四步建书工作流、多草稿与建书历史、基础章节版本历史、逐章生成正文候选、续写与重写、多候选保存及现稿对照、新版 JSON 作品及建书记录导入导出。未实现：旧版数据迁移、桌面容器、无人值守批量生文、榜单、拆书、每日统计、图片生成和逐字差异对比。Agnes 已按官方接口文档加入预设与连接测试，真实账号调用仍待验证。
