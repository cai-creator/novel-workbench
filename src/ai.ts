...
export async function generateDraft(args: {
  ...
  try {
    response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: `你是小说共创写手。保持人物和设定一致。${task[mode]}` },
          { role: 'user', content: `${brief}\n\n本次要求：${instruction.trim()}` },
        ],
        stream: false,
      }),
      signal,
    ...
    throw new Error(`连接模型失败：${error instanceof Error ? error.message : String(error)}。请检查地址、网络或浏览器跨域限制。`)
  }
  ...
  if (!content) throw new Error('模型没有返回可用正文。请检查模型 ID 和输出参数。')
  return content
}
